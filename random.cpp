...
randomClass::randomClass(long sd)		//constructor with seed input
{
	if( sd )	seed = sd;
		else	seed = (long) time(NULL);
}

double randomClass::real(void)		//random number generator from 0 to 1
{
	seed *= 22695477L ;
	seed += 1L;
...
double randomClass::randomInterval( Real rmin, Real rmax )		//random number generator from rmin to rmax
{
